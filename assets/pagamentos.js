...
async function loadKit(){
  ...
  if(!id){ kitBox.textContent = "Nenhum kit."; btnCreateOrder.disabled=true; return; }
  const { data, error } = await supabase.from("kits").select("*").eq("id", id).single();
  if(error || !data){ kitBox.textContent = "Kit não encontrado."; btnCreateOrder.disabled=true; return; }
  kit = data;
...
async function applyCoupon(){
  ...
  if(!code){ couponMsg.textContent = "Cupom vazio."; recalc(); return; }
