...
btnSaveSettings.addEventListener("click", async ()=>{
  ...
  const { error } = await supabase
    .from("settings")
    .upsert({ key:"public", pix_key: pixKey.value.trim(), pix_message: pixMessage.value.trim() }, { onConflict:"key" });
...
btnNewKit.addEventListener("click", ()=>{
  ...
  kitId.value=""; kitName.value=""; kitPrice.value=""; kitImage.value=""; kitDesc.value=""; kitActive.value="true";
  kitMsg.textContent="";
});
...
btnSaveKit.addEventListener("click", async ()=>{
  ...
  if(!payload.name){ kitMsg.innerHTML=`<span style="color:var(--bad)">Nome obrigatório.</span>`; btnSaveKit.disabled=false; return; }
...
async function loadKits(){
  ...
  kitsTbody.innerHTML="";
  if(error){ kitsTbody.innerHTML=`<tr><td colspan="4">${esc(error.message)}</td></tr>`; return; }
  for(const k of (data||[])){
...
btnNewCoupon.addEventListener("click", ()=>{
  ...
  couponId.value=""; couponCode.value=""; couponType.value="percent"; couponValue.value="";
  couponMin.value=""; couponMax.value=""; couponExpire.value=""; couponActive.value="true";
...
btnSaveCoupon.addEventListener("click", async ()=>{
  ...
  if(!code){ couponMsg.innerHTML=`<span style="color:var(--bad)">Código obrigatório.</span>`; btnSaveCoupon.disabled=false; return; }
  if(val<=0){ couponMsg.innerHTML=`<span style="color:var(--bad)">Valor inválido.</span>`; btnSaveCoupon.disabled=false; return; }
...
async function loadCoupons(){
  ...
  if(error){ couponsTbody.innerHTML=`<tr><td colspan="5">${esc(error.message)}</td></tr>`; return; }
  for(const c of (data||[])){
  ...
  couponsTbody.querySelectorAll("[data-editc]").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      couponValue.value = data.type==="percent" ? (data.value_percent||"") : ((data.value_cents||0)/100).toFixed(2);
...
async function loadOrders(){
  ...
  if(error){ ordersTbody.innerHTML=`<tr><td colspan="7">${esc(error.message)}</td></tr>`; return; }
